...
class BinanceFuturesClient:
    ...
    def _make_request(self, method: str, endpoint: str, data: typing.Dict):
        if method == "GET":
            try:
                ...
                logger.error(
                    "Connection error while make %s request to %s: %s", method, endpoint, e)
                return None
        elif method == "POST":
            try:
    ...
    def get_bid_ask(self, contract: Contract) -> typing.Dict[str, float]:
        data = dict()
        data['symbol'] = contract.symbol
        data = self._make_request("GET", "/fapi/v1/ticker/bookTicker", data)

        if data is not None:
            if contract.symbol not in self.prices:
                self.prices[contract.symbol] = {'bid': float(
                    data['bidPrice']), 'ask': float(data['askPrice'])}
            else:
                self.prices[contract.symbol]['bid'] = float(data['bidPrice'])
                self.prices[contract.symbol]['ask'] = float(data['askPrice'])
    ...
    def subscribe_channel(self, contracts: typing.List[Contract], channel: str):
        ...
        try:
            ...
            logger.error(
                "Websocket error while subscribing to %s %s updates: %s", len(contracts), channel, e)
