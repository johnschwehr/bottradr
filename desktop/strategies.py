...
class Strategy:
    def __init__(self, client: typing.Union["BitmexClient", "BinanceFuturesClient"], contract: Contract, exchange: str, timeframe: str, balance_pct: float, take_profit: float, stop_loss: float, strat_name: str):
        self.client = client
    ...
    def parse_trades(self, price: float, size: float, timestamp: int) -> str:
        ...
        if timestamp_diff >= 2000:
            logger.warning("%s %s: %s milliseconds of difference betweeen the current time and the trade time",
        ...
        # Missing Candle(s)
            ...
            logger.info("%s missing %s candles for %s %s (%s %s)", self.exchange,
                        missing_candles, self.contract.symbol, self.tf, timestamp, last_candle.timestamp)
